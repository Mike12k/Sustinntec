...
// Import required libraries https://github.com/ioticos/esp32_esp8266_post_request/blob/master/src/main.cpp
#ifdef ESP32
  #include <WiFi.h>
...
<body>
  ...
  <p>
    <i class="fas fa-thermometer-half" style="color:#059e8a;"></i> 
    <span class="ds-labels">Caudalimetro</span>
    <span id="caudal">%CAUDAL%</span>
    <sup class="units">&deg;litros</sup>
  </p>
</body>
<script>
...
setInterval(function ( ) {
  ...
  xhttp.open("GET",
GitHub
ioticos/esp32_esp8266_post_request
Contribute to ioticos/esp32_esp8266_post_request development by creating an account on GitHub.
"/temperaturef", true);
  xhttp.send();
}, 10000) ;
setInterval(function ( ) {
  ...
  xhttp.onreadystatechange = function() {
    if (this.readyState == 4 && this.status == 200) {
      document.getElementById("caudal").innerHTML = this.responseText;
    }
  };
  xhttp.open("GET", "/caudal", true);
  xhttp.send();
}, 10000) ;
</script>
</html>)rawliteral";
...
void setup(){
  ...
  server.on("/temperaturef", HTTP_GET, [](AsyncWebServerRequest *request){
   ...
   server.on("/caudal", HTTP_GET, [](AsyncWebServerRequest *request){
    request->send_P(200, "text/plain", readCaudal().c_str());
  });
  // Start server
  server.begin();
}
